...
class SubscriptionFeedbackSubcategoryFragment :
    SubscriptionFeedbackFragment(R.layout.content_feedback_subcategory) {
    ...
    override fun onViewCreated(
        ...
        val category =
            requireArguments().getSerializable(EXTRA_CATEGORY) as SubscriptionFeedbackCategory
        val layoutInflater = LayoutInflater.from(context)

        feedbackSubCategoryProvider.getSubCategories(category).forEach {
            val itemBinding = RowOneLineListItemBinding.inflate(
                layoutInflater,
                binding.subcategoryContainer,
                false
            )
    ...
    companion object {
        ...
        ): SubscriptionFeedbackSubcategoryFragment =
            SubscriptionFeedbackSubcategoryFragment().apply {
                val args = Bundle()
