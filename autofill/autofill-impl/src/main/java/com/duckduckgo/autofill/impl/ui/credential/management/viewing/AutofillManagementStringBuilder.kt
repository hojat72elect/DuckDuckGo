    ...
    override suspend fun stringForDeletePasswordDialogConfirmationMessage(numberToDelete: Int): String {
        ...
        val secondMessage = if (numberToDelete == 1) {
            ...
            context.resources.getQuantityString(
                R.plurals.credentialManagementDeleteAllSecondInstructionPlural,
                numberToDelete
            )
        }
        ...
        return withContext(dispatchers.io()) {
            return@withContext if (deviceSyncState.isUserSignedInOnDevice()) {
                if (numberToDelete == 1) {
                    ...
                    getQuantityString(
                        R.plurals.credentialManagementDeleteAllPasswordsFirstInstructionSyncedPlural,
                        numberToDelete
                    )
                }
            } else {
                if (numberToDelete == 1) {
                    ...
                    getQuantityString(
                        R.plurals.credentialManagementDeleteAllPasswordsDialogFirstInstructionNotSyncedPlural,
                        numberToDelete
                    )
                }
            }
