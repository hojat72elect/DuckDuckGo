    ...
    fun onIntentReceived(intent: Intent?, defaultColor: Int, isExternal: Boolean) {
        viewModelScope.launch(dispatcherProvider.io()) {
            runCatching {
                ...
                val toolbarColor =
                    intent?.getIntExtra(CustomTabsIntent.EXTRA_TOOLBAR_COLOR, defaultColor)
                        ?: defaultColor
                val isEmailProtectionLink =
                    emailProtectionLinkVerifier.shouldDelegateToInContextView(intentText, true)
