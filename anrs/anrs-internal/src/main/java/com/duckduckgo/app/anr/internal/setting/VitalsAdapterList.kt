...
class VitalsAdapterList : Adapter<ViewHolder>() {
    ...
    override fun onCreateViewHolder(
        ...
        return when (viewType) {
            ITEM_VIEW_TYPE_ANR -> {
                val binding =
                    ItemAnrBinding.inflate(LayoutInflater.from(parent.context), parent, false)
                ANRViewHolder(binding)
            }

            ITEM_VIEW_TYPE_CRASH -> {
                val binding =
                    ItemCrashBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    open class ANRViewHolder(open val binding: ItemAnrBinding) :
        RecyclerView.ViewHolder(binding.root)

    open class CrashViewHolder(open val binding: ItemCrashBinding) :
        RecyclerView.ViewHolder(binding.root)
