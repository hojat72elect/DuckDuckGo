...
class CrashAnrsObserver @Inject constructor(
    ...
    private fun observeAnrDatabase() = appCoroutineScope.launch(coroutineDispatcher.io()) {
        ...
        anrDatabase.arnDao().getAnrsFlow().drop(1).distinctUntilChanged()
            .flowOn(coroutineDispatcher.io()).collect { anrs ->
            anrs.forEach { anr ->
    ...
    private fun observeCrashDatabase() = appCoroutineScope.launch(coroutineDispatcher.io()) {
        ...
        uncaughtExceptionDao.allFlow().drop(1).distinctUntilChanged()
            .flowOn(coroutineDispatcher.io()).collect { crashes ->
