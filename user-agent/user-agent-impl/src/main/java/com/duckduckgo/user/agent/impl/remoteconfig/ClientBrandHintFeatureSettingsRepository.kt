...
class RealClientBrandHintFeatureSettingsRepository @Inject constructor(
    ...
    override fun updateAllSettings(settings: ClientBrandHintSettings) {
        ...
        dao.updateAllDomains(settings.domains.map {
            ClientHintBrandDomainEntity(
                it.domain,
                it.brand.name
            )
        })
    ...
    private fun loadToMemory() {
        ...
        clientBrandHints.addAll(clientBrandHintsDomainList.map {
            ClientBrandHintDomain(
                it.url,
                ClientBrandsHints.from(it.brand)
            )
        })
    }
}
